...
def extract_daterange_eachhour(Df,hour_start,startday=None,endday=None):
    ...
    return pd.concat([Df_extr,Df_extr2,Df_extr3,Df_extr4,Df_extr5,Df_extr6],axis=1).transpose().sort_index()
...
ts = pd.DatetimeIndex(Df_new.datetime)
Df_new.index = ts
for i,curlab in enumerate(fewlabels):
    Df_new[curlab] = probas[:,i]


df_site = extract_av_acrossdays(Df_new)
